...
const UserHome = () => {
    ...
    useEffect(() => {
        if (isUserLogin) {
            ...
            if (search) {
                dispatch(searchUsers({ firstName: search }));
            }
        }
    }, [isUserLogin, search, dispatch]);
            ...
            <div className='w-full bg-slate-100 shadow-2xl shadow-slate-900 rounded-t-lg h-24 flex items-center justify-between border-b z-10'>
                ...
                <div className='w-[60%]'>
                    <ul className='left-0 text-decoration: none flex justify-between items-center'>
                        <li><HiUserGroup className='text-blue-950 w-12 h-12 p-3 hover:opacity-80 hover:bg-gray-300 hover:text-red-800 cursor-pointer rounded-full' onClick={handleNavigateGroup} /></li>
                        <li><SiInstatus className='text-blue-950 w-12 h-12 p-3 hover:opacity-80 hover:bg-gray-300 hover:text-red-800 cursor-pointer rounded-full' onClick={handleNavigateStatus} /></li>
                        <li><BsFillChatLeftDotsFill className='text-blue-950 w-12 h-12 p-3 hover:opacity-80 hover:bg-gray-300 hover:text-red-800 cursor-pointer rounded-full' /></li>
                        <li><UserHomeDropDownMenu /></li>
                    </ul>
                </div>
            </div>
                ...
                <div className='rounded-bl-lg min-w-28 w-[40%] h-full bg-gradient-to-b from-slate-100 to-slate-50 border-r'>
                    <div className='sticky top-0 flex justify-between h-28 shadow-slate-600 shadow-md'>
                        <div className='relative w-full pt-1 top-0 bg-slate-200'>
                            <input 
                                className='w-[94%] h-8 rounded mx-2 mt-3 pl-10 right-2 text-blue-950'
                                type="text"
                                placeholder="search..."
                    ...
                    <div className='max-h-[90%] h-[84%] overflow-clip hover:overflow-y-scroll'>
                        {(search || showUserList) && searchResults.map((item, index) => (
                            <div onClick={() => handleClickOnCreateChat(item)} key={index}>
                                <ChatCard item={item} />
                            </div>
                        ))}
                    </div>
                </div>
                <div className='w-[60%] h-full relative flex flex-col rounded-br-lg'>
                    {query && selectedUserId ? (
                        <div className='h-30 w-full rounded-b-lg overflow-scroll'>
                            ...
                            <div className='w-full h-[84%] overflow-y-scroll overflow-clip px-[5%]'>
                                <div className='flex flex-col justify-center py-20 space-y-10'>
                                    {[...Array(15)].map((item, id) => (
                                        <MessageCard key={id} isRegUserMessage={id % 2 === 0} content="message" />
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className='flex items-center justify-center text-white object-cover h-full w-full text-center' style={{ backgroundImage: `url('https://cdn.pixabay.com/photo/2016/08/11/23/48/mountains-1587287_1280.jpg')` }}>
